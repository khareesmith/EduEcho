...
This setup contains an example of how to implement RAG support in applications that use voice as their user interface, powered by the GPT-4o realtime API for audio. The pattern is described in more detail in [this blog post](https://aka.ms/voicerag), and a sample app from Microsoft can be found in [this short video](https://youtu.be/vXJka8xZ9Ko).
...
This template is mainly set up for [local development](#local-environment).
...
2. Clone the repo (`git clone https://github.com/khareesmith/VoiceRAGTutor.git`)
...
4. The app is available on [http://localhost:8765](http://localhost:8765).
   ...
   To try out the app, click the "Start conversation button", say "Hello", and then ask a question about your data.
